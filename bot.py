...
def get_user_by_tg_id(tg_id: int):
    """Получает актуальные данные профиля и баланса по Telegram ID."""
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cursor:
        ...
        logging.error(f"Ошибка получения профиля по TG ID {tg_id}: {e}")
        return None
...
def get_main_menu_keyboard():
    """Создает постоянную реплай-клавиатуру внизу экрана."""
                    ...
                    with conn.cursor() as cursor:
                        ...
                        if user_data:
                            ...
                            success_msg = (
                                ...
                                f"Теперь вам доступны уведомления, а баланс отображается прямо в меню ниже!"
                            )
                            await message.answer(success_msg, parse_mode="Markdown", reply_markup=get_main_menu_keyboard())
                            return
                        else:
...
# =====================================================================
# ХЕНДЛЕР ПРОСМОТРА БАЛАНСА И ПРОФИЛЯ
# =====================================================================
@dp.message(F.text == "👤 Мой профиль")
@dp.message(Command("profile"))
async def cmd_profile(message: types.Message):
    """Выводит актуальную информацию о пользователе из базы данных сайта."""
    user_data = get_user_by_tg_id(message.from_user.id)
    
    if not user_data:
        await message.answer(
            "❌ Ваш Telegram-аккаунт еще не привязан к профилю на сайте.\n\n"
            f"Чтобы привязать его, зайдите на сайт **{WEBSITE_URL}**, перейдите в раздел **Профиль** и нажмите кнопку «Привязать аккаунт Telegram».",
    ...
    profile_text = (
        f"👤 Ваш профиль на Vest SMM\n\n"
        f"🔑 Логин: `{username}`\n"
        f"🆔 ID аккаунта: `#{user_id}`\n"
        f"💰 Баланс на сайте: {balance:.2f} ₽\n"
        f"🔔 Уведомления о заказах: {notif_status}\n\n"
        f"Пополнить баланс или заказать накрутку можно на сайте."
    )
    
    site_kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="🌐 Открыть панель накрутки", url=WEBSITE_URL)]
    ])
    await message.answer(profile_text, parse_mode="Markdown", reply_markup=site_kb)
...
async def msg_to_site(message: types.Message):
    ...
    await message.answer("Перейти к полному каталогу услуг и заказам:", reply_markup=site_kb)


# =====================================================================
# ИСПРАВЛЕНО: СБРОС ПАРОЛЯ ПО КНОПКЕ ИЛИ КОМАНДЕ
# =====================================================================
    ...
    try:
        with get_db_connection() as conn:
            with conn.cursor() as cursor:
                ...
                await message.answer(f"🔑 Найдена учетная запись: `{user_data[1]}`.\n\nВведите **Новый Пароль** для входа на сайт (не менее 6 символов):")
    except Exception as e:
        logging.error(f"Ошибка при вызове сброса пароля: {e}")
